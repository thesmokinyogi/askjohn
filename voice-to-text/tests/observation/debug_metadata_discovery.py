...
def observe_metadata_discovery():
    ...
    try:
        ...
        try:
            ...
            auth_attempts = [
                ("G.1: No explicit scopes (Gemini's code)", None),
                ("G.2: With explicit scopes", ['https://www.googleapis.com/auth/cloud-platform'])
            ]

            rest_success = False
            for attempt_name, scopes in auth_attempts:
                logger.info(f"\n      {attempt_name}:")
                try:
                    # Get credentials
                    if scopes:
                        ...
                        logger.info(f"         Using no explicit scopes")
                    ...
                    logger.info(f"         Trying next auth method...")
                    continue
                    ...
                    import traceback
                    logger.error(f"         {traceback.format_exc()}")
                    # Try next auth method on any error
                    continue

            if not rest_success:
                logger.error(f"\n      ✗ All REST API auth methods failed")
